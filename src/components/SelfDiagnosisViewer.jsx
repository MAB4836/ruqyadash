...
const SelfDiagnosisViewer = ({ onBack, isDarkMode, onNavigateToRuqyah }) => {
  ...
  useEffect(() => {
    const loadHtmlContent = async () => {
      try {
        const response = await fetch('/selfDiagnosis.html')
        if (!response.ok) {
        ...
        console.error('Error loading selfDiagnosis.html:', err)
      } finally {
        setLoading(false)
      }
    }
      ...
      <div className={`min-h-screen p-2 md:p-4 ${isDarkMode ? 'bg-gradient-to-br from-gray-900 to-blue-900' : 'bg-gradient-to-br from-blue-50 to-purple-50'}`}>
        <div className="w-full md:max-w-4xl md:mx-auto">
          ...
          <div className={`p-6 rounded-lg ${isDarkMode ? 'bg-red-900 text-red-200' : 'bg-red-50 text-red-800'}`}>
            ...
            <p>Failed to load selfDiagnosis.html: {error}</p>
          </div>
        </div>
      </div>
    )
...
export default SelfDiagnosisViewer